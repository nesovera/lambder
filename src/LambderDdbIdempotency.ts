...
export type LambderIdempotencyBeginResult =
    ...
    | { state: "pending" }
    | { state: "done"; statusCode: number; contentType: string | null; body: string };
...
export class LambderDdbIdempotency {
    ...
    async complete(
        scopeKey: string,
        ownerToken: string,
        { statusCode, contentType, body, ttlSeconds }:
        { statusCode: number, contentType: string | null, body: string, ttlSeconds: number },
    ): Promise<boolean> {
        const nowSeconds = Math.floor(Date.now() / 1000);
        try {
            await this.client.send(new PutItemCommand({
