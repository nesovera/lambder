 ...
 * or a binary body.
 */
const isReplayableAnswer = (answer: LambderApiAnswer): boolean =>
 ...
 */
const keyReusedRefusal = (apiName: string): LambderApiRefusal => new LambderApiRefusal(
 ...
 */
const requestFingerprintOf = async (payload: unknown): Promise<string> => {
...
export class LambderApiIdempotencyEngine {
    ...
    private async scopeOf(apiName: string, ctx: LambderApiCallContext, request: LambderApiRequest, key: string): Promise<string> {
        ...
        if(sessionKey) return joinKeyFields(await boundKeyField("s", sessionKey), apiName, key);
     ...
     */
    async resolveKeyedCall(apiName: string, request: LambderApiRequest, ctx: LambderApiCallContext): Promise<LambderIdempotentCall | null> {
     ...
     */
    async findReplay(apiName: string, call: LambderIdempotentCall, trace: LambderApiCallTrace): Promise<LambderApiAnswer | null> {
        const store = this.store;
        if(!store) return null;
        let done: LambderIdempotencyDoneRecord | null;
        try {
        ...
        if(!done) return null;
        if(done.fingerprint !== call.fingerprint) throw keyReusedRefusal(apiName);
        trace.replayed = true;
    ...
    async withIdempotency(
        apiName: string,
        { scopeKey, fingerprint }: LambderIdempotentCall,
        ...
        if(begun.state !== "new" && begun.fingerprint !== fingerprint) throw keyReusedRefusal(apiName);
        ...
        // exists to prevent.
        const settleClaim = async (answer: LambderApiAnswer, replayable: boolean) => {
            try {
                if(replayable){
                ...
                console.warn(
                    `Lambder idempotency: "${apiName}" ran and answered, but storing its record failed. ` +
                    "The caller keeps this answer; a retry under the same key replays it if the write landed after all, and executes again if it did not.",
                    storeErr,
                );
            }
        };
