...
export default class LambderSessionManager<SessionData = any> {
    ...
    public async updateSessionData(
        session: LambderSessionRecord<SessionData>,
        newData: SessionData,
    ): Promise<LambderSessionRecord<SessionData>> {
    ...
    public async renewSession(session: LambderSessionRecord<SessionData>): Promise<LambderSessionRecord<SessionData>|null>{
        ...
        if(needsWrite){
            ...
            await this.store.put(session).catch((err: unknown) => {
    ...
    private async deleteAllUnder(sessionKeyHash: string): Promise<void> {
    ...
    public async regenerateSession(session: LambderSessionRecord<SessionData>): Promise<LambderCreatedSession<SessionData>> {
        if(!session) throw new Error("Invalid session");
