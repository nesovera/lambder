 ...
 */
export const isMintedSessionToken = (token: string): boolean => {
...
export default class LambderSessionManager<SessionData = any> {
    ...
    private store: LambderSessionStore<SessionData>;
    ...
    private assertCryptoFitsStore(store: LambderSessionStore<SessionData>): void {
        if(!this.crypto.isCryptographic && !store.isMemoryOnly){
            throw new Error(
                "Lambder: this session crypto does not hash and does not draw cryptographically random bytes, " +
     ...
     */
    private sessionKeyHashOf(sessionKey: string): Promise<string> {
     ...
     */
    private hashToken(value: string): Promise<string> {
    ...
    public async createSession(
        ...
        const session: LambderSessionRecord<SessionData> = {
            sessionKeyHash,
     ...
     */
    public async lookupSession(sessionToken: string): Promise<LambderSessionRecord<SessionData>|null>{
        const [ sessionKeyHash, secret ] = sessionToken.split(":");
        if(!sessionKeyHash || !secret) return null;
        ...
        let session: LambderSessionRecord<SessionData> | null;
        try{
        ...
        if(!this.crypto.constantTimeEqual(session.sessionKeyHash ?? "", sessionKeyHash)) return null;
        if(!this.crypto.constantTimeEqual(session.secretHash ?? "", secretHash)) return null;
        ...
        if(typeof session.dataVersion !== "number") return null;
    ...
    public async renewSession(session: LambderSessionRecord<SessionData>): Promise<LambderSessionRecord<SessionData>|null>{
        ...
        try{
            const result = await this.store.update(session.sessionKeyHash, session.secretHash, { ...refreshed, ...slid },
                refreshed.data !== undefined ? { dataVersion: session.dataVersion } : undefined);
            if(result === "missing") return null;
            ...
            // leaves it.
            if(result === "updated") return refreshed.data !== undefined ? { ...renewed, dataVersion: session.dataVersion + 1 } : renewed;
     ...
     */
    public async refreshSessionData(session: LambderSessionRecord<SessionData>): Promise<LambderSessionRecord<SessionData>|null>{
     ...
     */
    public async isSessionTokenValid(session: LambderSessionRecord<SessionData> | null, sessionToken: string | null): Promise<boolean> {
        if(!session) return false;
        if(!sessionToken) return false;
        // Presented raw secrets are checked against the stored hashes.
        const [ sessionKeyHash, secret ] = sessionToken.split(":");
        if(!sessionKeyHash || !secret) return false;
        if(!this.crypto.constantTimeEqual(session.sessionKeyHash, sessionKeyHash)) return false;
        if(!this.crypto.constantTimeEqual(session.secretHash, await this.hashToken(secret))) return false;
     ...
     */
    public async isSessionCsrfTokenValid(session: LambderSessionRecord<SessionData> | null, csrfToken: string | null): Promise<boolean> {
        if(!session?.csrfTokenHash) return false;
    ...
    public async deleteSession(session: LambderSessionRecord<SessionData>): Promise<boolean>{
     ...
     */
    public async deleteSessionAll (session: LambderSessionRecord<SessionData>): Promise<boolean>{
     ...
     * can run it again.
     */
    private async deleteAllUnder(sessionKeyHash: string): Promise<boolean> {
        for(let pass = 0; pass < SUBJECT_DELETE_MAX_PASSES; pass++){
            let foundGone = false;
            await this.forEachSessionOf(sessionKeyHash, async (secretHash) => {
                if(await this.store.delete(sessionKeyHash, secretHash) === null) foundGone = true;
            });
            if(!foundGone) return true;
        }
     ...
     */
    private async forEachSessionOf(sessionKeyHash: string, write: (secretHash: string) => Promise<unknown>): Promise<void> {
