import { resolveCookieDomain, serializeCookie, serializeClearCookie, type LambderCookieOptions } from "../shared/wire/LambderCookie.js";
...
const rawCookieValue = (value: string): string => value;
...
export const assertSessionCookiePrefixes = (sessions: {
    ...
    if(sessions.cookieOptions.secure === false){
        ...
        if(hostPrefixed.length > 0){
            throw new Error(
                `Lambder: session cookie ${hostPrefixed.join(" and ")} uses the __Host- prefix, which a browser accepts only on a Secure cookie. ` +
        ...
        if(securePrefixed.length > 0){
            throw new Error(
                `Lambder: session cookie ${securePrefixed.join(" and ")} uses the __Secure- prefix, which a browser accepts only on a Secure cookie. ` +
                "Drop session.cookie.secure: false, or drop the prefix; keeping both means the browser discards the cookie silently and no session is ever read."
            );
        }
    }
    if(hostPrefixed.length === 0) return;
    if(sessions.cookieOptions.domain !== undefined){
        throw new Error(
            `Lambder: session cookie ${hostPrefixed.join(" and ")} uses the __Host- prefix, which a browser accepts only without a Domain. ` +
    ...
    if(sessions.cookieOptions.path !== undefined && sessions.cookieOptions.path !== "/"){
        throw new Error(
            `Lambder: session cookie ${hostPrefixed.join(" and ")} uses the __Host- prefix, which a browser accepts only at Path=/. ` +
...
export default class LambderSessionController<TSessionData = any> {
    ...
    private readonly ctx: LambderSessionCallSurface<TSessionData>;
    private readonly request: LambderSessionRequestInfo;
     ...
     */
    private writeSessionCookies(expiresAt: number, sessionToken: string, csrfToken: string | null): void {
        ...
        if(csrfToken !== null) this.ctx.responseHeaders.add("Set-Cookie", serializeCookie(this.csrfCookieKey, csrfToken, { ...scope, expires, maxAge, encode: rawCookieValue }));
    };
     ...
     */
    private addClearCookiePair(scope: LambderCookieOptions): void {
     ...
     */
    private cookieClearDomains(): (string | undefined)[] {
        ...
        for(let i = 0; i + 2 <= labels.length; i++){ scopes.push(labels.slice(i).join(".")); }
     ...
     */
    private clearSessionCookiesEverywhere(): void {
     ...
     */
    private refuseAmbiguousSession(what: string): never {
        console.warn(
            `Lambder session: ${what}, from ${this.request.host}. `
     ...
     */
    private scanSessionCookies(): { sessionTokens: string[]; csrfTokens: string[] } {
    ...
    private areRequestSessionTokensValid(sessionTokens: string[]): boolean {
        if(sessionTokens.length === 0) return false;
        if(this.request.csrfToken !== null) return this.request.csrfToken.length > 0;
        return true;
    };

    async createSession (sessionKey: string, data?: TSessionData, ttlInSeconds?: number): Promise<LambderSessionRecord<TSessionData>> {
     ...
     */
    async issueSession (sessionKey: string, data?: TSessionData, ttlInSeconds?: number): Promise<LambderCreatedSession<TSessionData>> {
    ...
    async regenerateSession (): Promise<LambderSessionRecord<TSessionData>> {
     ...
     */
    async reissueSession (): Promise<LambderCreatedSession<TSessionData>> {
        if(!this.ctx.session) throw new LambderSessionNotFoundError();
        const created = await this.manager.regenerateSession(this.ctx.session);
        if(!created) this.endWithNoSession();
        this.setSessionCookies(created);
        this.ctx.session = created.session;
        return created;
    };

    async fetchSession (): Promise<LambderSessionRecord<TSessionData>>{
        const { sessionTokens: candidates, csrfTokens } = this.scanSessionCookies();
        if(!this.areRequestSessionTokensValid(candidates)){ throw new LambderSessionNotFoundError("Session tokens are invalid"); }
        ...
        // used.
        const live: { token: string; session: LambderSessionRecord<TSessionData> }[] = [];
        ...
        const found = live[0];
        if(!found) this.endWithNoSession();
        ...
        if(!session) this.endWithNoSession();
        ...
        if(evictsHostOnlyTwin) this.clearSessionCookies(true);
        ...
        if(evictsHostOnlyTwin || session.expiresAt !== expiresBefore) await this.slideSessionCookies(session, found.token, csrfTokens);
     ...
     */
    private async slideSessionCookies(session: LambderSessionRecord<TSessionData>, sessionToken: string, csrfTokens: string[]): Promise<void> {
     ...
     */
    private endWithNoSession(): never {
        this.ctx.session = null;
        throw new LambderSessionNotFoundError();
    };

    async fetchSessionIfExists (): Promise<LambderSessionRecord<TSessionData>|null> {
        try {
            ...
            if(err instanceof LambderSessionNotFoundError) return null;
            throw err;
        }
    };
     ...
     */
    async updateSessionData (newData: TSessionData): Promise<LambderSessionRecord<TSessionData>> {
        if(!this.ctx.session) throw new LambderSessionNotFoundError();
        const updated = await this.manager.updateSessionData(this.ctx.session, newData);
        if(!updated) this.endWithNoSession();
     ...
     */
    async refreshSessionData (): Promise<LambderSessionRecord<TSessionData>> {
        if(!this.ctx.session) throw new LambderSessionNotFoundError();
        const refreshed = await this.manager.refreshSessionData(this.ctx.session);
        if(!refreshed) this.endWithNoSession();
    ...
    async endSessionAll (): Promise<void> {
        if(!this.ctx.session) throw new LambderSessionNotFoundError();
