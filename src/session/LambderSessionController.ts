...
export default class LambderSessionController<TSessionData = any> {
    ...
    private writeSessionCookies(expiresAt: number, sessionToken: string, csrfToken: string | null): void {
        ...
        if(csrfToken !== null) this.ctx.responseHeaders.add("Set-Cookie", serializeCookie(this.csrfCookieKey, csrfToken, { ...scope, expires, encode: rawCookieValue }));
    };
    ...
    async fetchSession (): Promise<LambderSessionRecord<TSessionData>>{
        ...
        const found = live[0];
        if(!found) throw new LambderSessionNotFoundError();
        ...
        if(!session) throw new LambderSessionNotFoundError();
    ...
    async fetchSessionIfExists (): Promise<LambderSessionRecord<TSessionData>|null> {
        try {
            ...
            if(err instanceof LambderSessionAmbiguousError) return null;
            throw err;
        }
    };
     ...
     */
    async refreshSessionData (): Promise<LambderSessionRecord<TSessionData>|null> {
        if(!this.ctx.session) throw new LambderSessionNotFoundError();
