...
export class LambderApiIdempotencyEngine {
    private store: LambderDdbIdempotency | null = null;
    private defaultTtlSeconds = 24 * 3600;
    private failOpen = true;

    configure(config: LambderApiIdempotencyConfig): void {
        if(this.store) throw new Error("Lambder: enableApiIdempotency() was already called.");
     ...
     */
    private readKey(ctx: LambderRenderContext): string | null {
     ...
     */
    private scopeOf(apiName: string, ctx: LambderRenderContext, key: string): string {
    ...
    async findReplay(apiName: string, ctx: LambderRenderContext): Promise<LambderResponse | null> {
        ...
        try {
            ...
            return new LambderResponse({
                statusCode: done.statusCode as HttpStatusCode,
    ...
    async withIdempotency(
        ...
        try {
            ...
            if(this.failOpen) return await exec();
            throw err;
        }
        ...
        const settleClaim = async (response: LambderResponse) => {
            ...
            try {
                if(cacheable){
                    ...
                    for(const header of execSetHeaders) storedResponse.setHeader(header.key, header.value);
                    for(const header of execAddHeaders) storedResponse.addHeader(header.key, header.value);
                ...
                if(!this.failOpen) throw storeErr;
            }
        };
        ...
        try {
            ...
            catch(cleanupErr){ if(!this.failOpen) throw cleanupErr; }
            throw err;
        }
    }
}
