 ...
 */
const fetchOrExplain = async (url: string, init: RequestInit): Promise<Response> => {
    try {
        ...
        if(failedToParseUrl && url.startsWith("/") && typeof globalThis.location?.href !== "string"){
            throw new LambderTransportFailure(
                "protocol",
                `lambderFetchTransport could not resolve the relative apiPath "${url}": there is no page to resolve it against outside a browser. `
                + "Give the caller an absolute apiPath (https://api.example.com/api), or a transport that does not need one.",
                { cause: err },
            );
        }
        throw err;
 ...
 */
export const lambderFetchTransport = (options: { cors?: boolean } = {}): LambderApiTransport => async (request) => {
    ...
    if(cors === undefined){
        ...
        try { cors = new URL(request.apiPath, globalThis.location?.href).origin !== globalThis.location?.origin; }
        catch { cors = false; }
    }
