...
type MemoryIdempotencyRecord =
    | { state: "pending"; ownerToken: string }
...
export class LambderMemoryIdempotencyStore implements LambderIdempotencyStore {
    ...
    async complete(
        ...
        this.records.set(
            scopeKey,
            { state: "done", ownerToken, statusCode, headers: structuredClone(headers), body },
    ...
    async abandon(scopeKey: string, ownerToken: string): Promise<void> {
        const existing = this.records.get(scopeKey);
        if(existing && existing.ownerToken === ownerToken) this.records.delete(scopeKey);
    }
