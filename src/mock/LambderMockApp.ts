...
> {
    ...
    private createContext(request: LambderApiRequest): LambderMockCallContext<S> {
        ...
        const chargeRateLimit = async (policy: string, key: string | undefined, refuse: boolean) => {
            ...
            if(refuse && refusal) throw refusal;
            return checkResult;
        };
        bindCallTools(ctx, {
            getters: {
                sessionController: () => {
                    if(!pipeline.hasSessions) throw new Error(`LambderMockApp: ctx.sessionController on "${request.apiName}" needs the sessions option at creation.`);
    ...
    transport(options: LambderMockTransportOptions = {}): LambderMockTransport {
        ...
        // and be signed out.
        if(cookies !== "document") return transportCarrying(async (request) => await withJar({ ...request, token: "" }), jar);
