 ...
 */
const isRestNotMockedEntry = (slice: object): slice is LambderMockRestEntry =>
    typeof (slice as LambderMockRestEntry).restNotMockedReason === "string";
 ...
 */
export class LambderMockEntryRegistry<C> {
    private readonly entries = new Map<string, LambderMockEntry<C, any>>();
    /**
     ...
     */
    private readonly overrideStacks = new Map<string, LambderMockEntry<C, any>[]>();
    /** The reason register() was given a rest entry with; null while it was given none. */
    private restReason: string | null = null;
    ...
    addSlices(slices: readonly (Record<string, LambderMockEntry<C, any>> | LambderMockRestEntry)[]): void {
        const staged = new Map<string, LambderMockEntry<C, any>>();
        let stagedRestReason: string | null = null;
        ...
        for(const [name, entry] of staged) this.entries.set(name, entry);
        if(stagedRestReason !== null) this.restReason = stagedRestReason;
    }
    ...
    pushOverride(name: string, entry: LambderMockEntry<C, any>): LambderMockOverride {
        ...
        const restore = () => {
            ...
            current.splice(at, 1);
            if(!current.length) this.overrideStacks.delete(name);
        };
        return { restore } satisfies LambderMockOverride;
    }
