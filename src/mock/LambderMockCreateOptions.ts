...
type LambderContractWindowOverrideNames<C> =
    { [K in keyof C]: LambderWindowOverrideNamesIn<LambderContractRateLimitOf<C, K>> }[keyof C] & string;

type LambderWindowOverrideNamesIn<R> =
    R extends string | readonly string[] ? never
    : { [N in keyof R]: R[N] extends object ? ([Extract<keyof R[N], LambderRateLimitWindow>] extends [never] ? never : N) : never }[keyof R];
...
type LambderContractIdempotentKeys<C> =
    { [K in keyof C]: [LambderContractIdempotencyOf<C, K>] extends [never] ? never : LambderContractIdempotencyOf<C, K> extends false ? never : K }[keyof C];
...
type LambderMockRateLimitsOptions<C, S, P extends LambderMockRateLimitPolicies<S>> = {
    policies: P
        ...
        // Keyed on the policy as written, so the error lands on its `per` or
        // `budget` rather than collapsing the whole policy to never.
        & { [N in keyof P & LambderContractRateLimitNames<C, "public">]: P[N] extends { per: "session" } ? { per: never } : unknown }
        & { [N in keyof P & LambderContractWindowOverrideNames<C>]: P[N] extends { budget: "perPolicy" } ? { budget: never } : unknown };
...
type LambderMockSessionsOption<C, S> =
    [LambderContractKeysWithMode<C, "session">] extends [never]
        ? {
...
type LambderMockIdempotencyOption<C, S, I> =
    [LambderContractIdempotentKeys<C>] extends [never]
        ? {
...
type LambderMockRateLimitsOption<C, S, P extends LambderMockRateLimitPolicies<S>> =
    [LambderContractRateLimitNames<C>] extends [never]
        ? {
...
export type LambderMockAppOptions<
    C, S, G,
...
> = LambderMockGuardsOption<C, S, G>
    & LambderMockSessionsOption<C, S>
    & LambderMockIdempotencyOption<C, S, I>
    & LambderMockRateLimitsOption<C, S, P>
    & {
 ...
 */
export type LambderMockTransport = LambderApiTransport & { readonly cookieJar: LambderCookieJar | null };
