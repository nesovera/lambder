...
export type GuardInputsOf<TEntry> = TEntry extends { guardInputs: infer G } ? G : never;
 ...
 */
export type LambderCallOptionsArg<TContract, TApiName, TProvided extends string, TOptions extends { guardInputs?: Record<string, unknown> }> =
    ...
    : TApiName extends keyof TContract
        ? [GuardInputsOf<TContract[TApiName]>] extends [never]
            ...
            : [keyof RemainingGuardInputs<TContract[TApiName], TProvided>] extends [never]
                ? [options?: TOptions & { guardInputs?: Partial<GuardInputsOf<TContract[TApiName]>> }]
                : [options: TOptions & {
