...
/**
 * The states of the HTML standard's tokenizer that decide where a value
 * lands, some of them merged where they read on alike:
 * - "betweenAttributes" stands for the before and after attribute name
 *   states, the after quoted value state and the self-closing start tag
 *   state. Its attributeName tells them apart where they differ: the
 *   attribute an "=" gives a value to, or "" where an "=" starts a new
 *   attribute.
 * - "comment" stands for every comment state, the ones the comment may be in
 *   kept in commentStates.
 * - The "rawText" states read the content of TEXT_CONTENT_ELEMENTS up to the
 *   element's own end tag (script data, RAWTEXT and RCDATA), with
 *   scriptLevel for a script's escaped and double escaped states.
 * - "markupOpen" and "markupOpenDash" are "<!" and "<!-" read, before a
 *   second "-" makes them a comment. "bogusComment" covers a DOCTYPE too,
 *   which also ends at its first ">".
 * "undecidable" is where the reader has lost track (see undecidable).
 */
type ReaderState =
    | "data" | "tagOpen" | "endTagOpen" | "markupOpen" | "markupOpenDash"
    | "tagName" | "betweenAttributes" | "attributeName" | "beforeValue" | "unquotedValue" | "quotedValue"
    | "bogusComment" | "comment"
    | "rawText" | "rawTextLessThan" | "rawTextEndTagOpen" | "rawTextEndTagName" | "rawTextDash" | "rawTextDashDash"
    | "scriptEscapeStart" | "scriptEscapeStartDash" | "scriptDoubleEscapeStart" | "scriptDoubleEscapeEnd"
    | "undecidable";
 ...
 * reads it.
 */
const TEXT_CONTENT_ELEMENTS: ReadonlySet<string> = new Set(["script", "style", "xmp", "iframe", "noembed", "noframes", "title", "textarea"]);
...
const nextCommentState = (state: number, character: string): number => {
    if(character === ">") return state === COMMENT_TEXT || state === COMMENT_END_DASH ? COMMENT_TEXT : COMMENT_CLOSED;
    if(character === "-"){
        if(state === COMMENT_START) return COMMENT_START_DASH;
...
export class HtmlPositionReader {
    private state: ReaderState = "data";
    ...
    private blame = "";
    private undecidableReason: HtmlUndecidable["reason"] | null = null;
            ...
            switch(this.state){
                ...
                case "tagOpen":
                    if(character === "!" || character === "/"){
                        ...
                        index += 1;
                    }else if(ASCII_LETTER.test(character)) this.openTag(false);
                    else this.state = character === "?" ? "bogusComment" : "data";
                    break;
                case "endTagOpen":
                    if(ASCII_LETTER.test(character)) this.openTag(true);
                    else if(character === ">"){ this.state = "data"; index += 1; }
                    else this.state = "bogusComment";
                    break;
                case "markupOpen":
                case "markupOpenDash":
                    if(character !== "-"){
                        ...
                        if(this.state === "markupOpen" && CDATA_OPENER.startsWith(html.slice(index, index + CDATA_OPENER.length))){
                            ...
                            if(close === -1 || !html.startsWith("]]", close - 2)) this.foreignPartedBy = "![CDATA[ ... ]]";
                        }
                        this.state = "bogusComment";
                        break;
                    }
                    index += 1;
                    if(this.state === "markupOpen") this.state = "markupOpenDash";
                    else { this.state = "comment"; this.commentStates = COMMENT_START; }
                    break;
                case "tagName":
                    if(HTML_WHITESPACE.test(character) || character === "/"){ this.enterBetweenAttributes(""); index += 1; }
                    else if(character === ">"){ this.closeTag(); index += 1; }
                    else {
                ...
                case "betweenAttributes":
                    if(HTML_WHITESPACE.test(character)){ index += 1; break; }
                    if(character === "=" && this.equalsAmbiguous){ this.loseTrack("attributeEquals"); break; }
                    this.equalsAmbiguous = false;
                    if(character === "/"){ this.attributeName = ""; index += 1; }
                    else if(character === ">"){ this.closeTag(); index += 1; }
                    else if(character === "="){
                        // With no name waiting for a value, an "=" starts
                        // the next attribute's name.
                        if(this.attributeName === ""){ this.state = "attributeName"; this.attributeName = "="; }
                        else this.state = "beforeValue";
                        index += 1;
                    }else { this.state = "attributeName"; this.attributeName = ""; }
                    break;
                case "attributeName":
                    if(HTML_WHITESPACE.test(character)){ this.enterBetweenAttributes(this.attributeName); index += 1; }
                    else if(character === "/"){ this.enterBetweenAttributes(""); index += 1; }
                    else if(character === ">"){ this.closeTag(); index += 1; }
                    else if(character === "="){ this.state = "beforeValue"; index += 1; }
                    else {
                    ...
                    break;
                case "beforeValue":
                    if(character === '"' || character === "'"){ this.state = "quotedValue"; this.quote = character; }
                    else if(character === ">") this.closeTag();
                    else if(!HTML_WHITESPACE.test(character)) this.state = "unquotedValue";
                    index += 1;
                    break;
                case "unquotedValue":
                    if(HTML_WHITESPACE.test(character)) this.enterBetweenAttributes("");
                    else if(character === ">") this.closeTag();
                    index += 1;
                    break;
                case "quotedValue": {
                ...
                case "comment": {
                    ...
                    for(let commentState = COMMENT_START; commentState <= COMMENT_END_BANG; commentState <<= 1){
                        if(!(this.commentStates & commentState)) continue;
                        const next = nextCommentState(commentState, html[index]!);
                        if(next === COMMENT_CLOSED) closes = true;
                        else nextStates |= next;
                    }
                    index += 1;
                    if(closes && nextStates !== 0) this.loseTrack("commentValue");
                    else if(closes) this.state = "data";
                    else this.commentStates = nextStates;
                    break;
                }
                case "rawText": {
                    ...
                    // well as on a "<".
                    let stop = -1;
                    if(this.scriptLevel === 0) stop = html.indexOf("<", index);
                    else {
                ...
                case "rawTextDashDash":
                    if(character === "<") this.state = "rawTextLessThan";
                    else if(character === "-") this.state = "rawTextDashDash";
                    else {
                        // "-->" ends a script's escaped content.
                        if(character === ">" && this.state === "rawTextDashDash") this.scriptLevel = 0;
                        this.state = "rawText";
                    }
                    index += 1;
                    break;
                case "rawTextLessThan":
                    if(character === "/"){
                        ...
                        if(ASCII_LETTER.test(character) || character === "!" || character === "?") this.noteMarkupInText();
                        this.state = "rawText";
                    }
                    break;
                case "rawTextEndTagOpen":
                    if(ASCII_LETTER.test(character)) this.state = "rawTextEndTagName";
                    else { this.noteMarkupInText(); this.state = "rawText"; }
                    break;
                case "rawTextEndTagName":
                    if(ASCII_LETTER.test(character)){ this.nameBuffer += character.toLowerCase(); index += 1; }
                        ...
                        // read like any tag's.
                        this.endTag = true;
                        if(character === ">") this.closeTag();
                        else this.enterBetweenAttributes("");
                        index += 1;
                    }else { this.noteMarkupInText(); this.state = "rawText"; }
                    break;
                case "scriptEscapeStart":
                case "scriptEscapeStartDash":
                    if(character !== "-"){ this.state = "rawText"; break; }
                    index += 1;
                    if(this.state === "scriptEscapeStart") this.state = "scriptEscapeStartDash";
                    else { this.state = "rawTextDashDash"; this.scriptLevel = 1; }
                    break;
                case "scriptDoubleEscapeStart":
                case "scriptDoubleEscapeEnd":
                    if(ASCII_LETTER.test(character)){ this.nameBuffer += character.toLowerCase(); index += 1; break; }
                    if(HTML_WHITESPACE.test(character) || character === "/" || character === ">"){
                        ...
                        if(this.nameBuffer === "script") this.scriptLevel = this.state === "scriptDoubleEscapeStart" ? 2 : 1;
                        index += 1;
                    }
                    this.state = "rawText";
                    break;
    ...
    point(): HtmlPoint {
        const point: HtmlPoint = { position: "tag", attributeName: "", quote: "", tagName: "" };
        ...
        switch(this.state){
            ...
            case "quotedValue":
                ...
                if(this.state === "quotedValue") point.quote = this.quote;
                break;
            case "rawText":
            case "rawTextDash":
            case "rawTextDashDash":
            ...
            case "scriptDoubleEscapeEnd":
                ...
                if(RAW_TEXT_ELEMENTS.has(this.tagName)){ point.position = "rawText"; point.tagName = this.tagName; }
                else if(this.state === "rawText") point.position = "text";
                break;
            default:
     ...
     */
    joinWith(other: HtmlPositionReader, label = ""): boolean {
        if(this.positionKey() !== other.positionKey() || this.foreignPartedBy !== other.foreignPartedBy) return false;
        if(this.state === "betweenAttributes" && !this.equalsAmbiguous){
            if(other.equalsAmbiguous){ this.equalsAmbiguous = true; this.blame = other.blame; }
            else if(other.attributeName !== this.attributeName){ this.equalsAmbiguous = true; this.blame = label; }
        }
        return true;
    }
    ...
    private openTag(endTag: boolean): void {
        this.state = "tagName";
        this.tagName = "";
        this.endTag = endTag;
    }

    private enterBetweenAttributes(attributeName: string): void {
     ...
     */
    private noteMarkupInText(): void {
        if(!RAW_TEXT_ELEMENTS.has(this.tagName)) this.foreignPartedBy = this.tagName;
    }

    private loseTrack(reason: HtmlUndecidable["reason"]): void {
 ...
 * sees it.
 */
export const valueLanguageOf = (attributeName: string): string | null => {
    if(attributeName.startsWith("on")) return "JavaScript";
    if(attributeName === "style") return "CSS";
    if(attributeName === "srcdoc") return "an HTML document";
    return null;
};
 ...
 */
export const safeUrlValue = (url: string, firstValueIndex: number): string => {
    if(firstValueIndex === -1 || /^[^&:/?#]*[:/?#]/.test(url.slice(0, firstValueIndex))) return url;
    // A URL parser drops C0 controls and spaces (U+0000 to U+0020) at both ends.
    let first = 0;
    let last = url.length;
    while(first < last && url.charCodeAt(first) <= 0x20) first += 1;
    while(last > first && url.charCodeAt(last - 1) <= 0x20) last -= 1;
    ...
    if(afterScheme === "&") return NEUTRALIZED_URL_VALUE;
    if(afterScheme === ":" && scheme !== "" && !SAFE_URL_SCHEMES.has(scheme.toLowerCase())) return NEUTRALIZED_URL_VALUE;
    return url;
};
