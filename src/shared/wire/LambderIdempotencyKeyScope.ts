...
export const IDEMPOTENT_ATTEMPT_NOT_SENT: LambderIdempotentAttemptOutcome = { ok: false, reason: "notSent" };
...
export const createIdempotencyKey = (): string => {
    ...
    if(!cryptoObj?.getRandomValues) throw new Error("createIdempotencyKey needs crypto.getRandomValues: an idempotency key must be unguessable, and this runtime offers no random source that is.");
...
let beginAttemptOf: (scope: LambderIdempotencyKeyScope) => LambderIdempotentAttempt;
...
export class LambderIdempotencyKeyScope {
     ...
     */
    #beginAttempt(): LambderIdempotentAttempt {
        const key = this.#key;
        this.#inFlight += 1;
        let settled = false;
        return { key, settle: (outcome) => {
            ...
            if(code === LAMBDER_REFUSAL_CODES.duplicateInFlight && this.#inFlight > 0) return;
 ...
 * ```
 */
export const createIdempotencyKeyScope = (): LambderIdempotencyKeyScope => new LambderIdempotencyKeyScope();

/** The attempt a call makes with its idempotencyKey option: a scope's current key, or a plain key with nothing to settle. */
export const beginIdempotentAttempt = (option: string | LambderIdempotencyKeyScope | undefined): LambderIdempotentAttempt =>
    option instanceof LambderIdempotencyKeyScope ? beginAttemptOf(option) : { key: option, settle: () => {} };
