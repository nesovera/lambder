 ...
 * fallback of its own.
 */
export const coerceToError = (value: unknown, fallbackMessage = "Unknown error"): Error => {
    if(value instanceof Error) return value;
    let message: string | undefined;
    if(typeof value === "string") message = value;
    else {
        try { message = JSON.stringify(value); } catch { try { message = String(value); } catch { message = undefined; } }
    }
