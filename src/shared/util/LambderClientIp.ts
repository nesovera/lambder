 ...
 */
const PORT_SUFFIXED_IP_HEADERS: ReadonlySet<string> = new Set(["cloudfront-viewer-address"]);
 ...
 */
export const rateLimitSubjectOf = (ip: string, ipv6PrefixLength: number = DEFAULT_IPV6_RATE_LIMIT_PREFIX): string => {
...
export const resolveClientIp = (
    ...
    for(const name of trustedClientIpHeaders){
        ...
        if(first) return normalizeClientIp(PORT_SUFFIXED_IP_HEADERS.has(header) ? first.replace(/:\d+$/, "") : first);
    }
