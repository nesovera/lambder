...
export const renderHtmlValue = (value: LambderHtmlValue): string => {
    if(value === null || value === undefined || typeof value === "boolean") return "";
    if(value instanceof LambderSafeHtml) return value.value;
...
const quoteTemplateEnd = (text: string): string => JSON.stringify(text.length > 40 ? `...${text.slice(-40)}` : text);

/** The interpolation at `index`, named in an error message by the template text just before it. */
const describeInterpolation = (strings: TemplateStringsArray, index: number): string =>
    `the interpolation after ${quoteTemplateEnd(strings.slice(0, index + 1).join("${...}"))}`;

export const html = (strings: TemplateStringsArray, ...values: LambderHtmlValue[]): LambderSafeHtml => {
    ...
    let urlValue: { url: string; firstValueIndex: number; quote: string; attributeName: string } | null = null;
    for(let index = 0; index < strings.length; index += 1){
        ...
        if(positions.undecidable){
            throw new Error(
                `html\`...\`: ${describeInterpolation(strings, index - 1)} is inside a comment, right before text that ends the comment ` +
        ...
        if(point.position === "afterEquals" || point.position === "unquoted"){
            throw new Error(
                `html\`...\`: ${describeInterpolation(strings, index)} is in an unquoted attribute position. ` +
                `Escaping cannot prevent injection there; quote the attribute: attr="\${value}".`,
            );
        }
        if(point.position === "tag"){
            throw new Error(
                `html\`...\`: ${describeInterpolation(strings, index)} is inside a tag where an attribute name goes. ` +
        ...
        if(valueLanguage){
            throw new Error(
                `html\`...\`: ${describeInterpolation(strings, index)} is inside the ${point.attributeName} attribute, whose value the browser ` +
        ...
        if(point.position === "rawText" && RAW_TEXT_ELEMENTS.has(point.tagName)){
            throw new Error(
                `html\`...\`: ${describeInterpolation(strings, index)} is inside a <${point.tagName}> element where HTML ` +
                `escaping does not apply. Pass data with the jsonScript() helper outside the element instead.`,
            );
        }
        if(point.position === "rawText"){
            throw new Error(
                `html\`...\`: ${describeInterpolation(strings, index)} comes after <${point.tagName}> content that inline SVG and ` +
        ...
        if(urlValue) urlValue.url += rendered;
        else out += rendered;
    }
    if(urlValue){
        throw new Error(
            `html\`...\`: ${describeInterpolation(strings, values.length - 1)} is inside a quoted ` +
            `${urlValue.attributeName} value that never closes.`,
        );
    }
    if(!positions.inPlainText){
        throw new Error(
            `html\`...\`: the template ${quoteTemplateEnd(strings.join("${...}"))} ends inside markup it opened (a tag, an attribute ` +
