...
const keepPercentSigns = (text: string): string => text.replaceAll("%", "%25");
 ...
 */
export const decodeRequestPath = (rawPath: string, alreadyDecoded: boolean): string =>
    rawPath.split("/").map(alreadyDecoded ? keepPercentSigns : decodeSegment).join("/");
...
export const decodePathParam = (value: string): string =>
    value.replace(/%2F|%25/gi, (escape) => (escape === "%25" ? "%" : "/"));
 ...
 * name holds.
 */
export const filePathOf = (path: string): string | null =>
    /%2F/i.test(path) ? null : path.replaceAll("%25", "%");
 ...
 * path on this one.
 */
export const encodePathForLocation = (path: string): string =>
    path.replace(/[^A-Za-z0-9\-._~!$&'()*+,;=:@/%]/gu, (character) => encodeURIComponent(character));
