...
1. **`servePublicFiles(options?)`**: a terminal slot that serves real files from its `source` (default: the `publicPath` folder; `LambderS3FileSource` for S3 and R2; or any `{ read(relativePath) }`, see "Public file sources" in the Readme). It runs only when no route or API matched, so unlike a `"/(.*)"` catch-all route it can never shadow routes registered after it. Traversal-safe, mime-typed, memory-cached for warm invocations, immutable Cache-Control for content-hashed assets (`app-4f8a1b2c.js`), automatic ETag/gzip. When the file does not exist, the request **falls through**.
