...
Nothing about it is invoke-specific except the guard, which is a written
authorization decision rather than a mechanism:

```typescript
// gateway-lambda/src/index.ts
import { initLambder, lambderGuard, refuse, describeCrash } from "lambder";
import { z } from "zod";

const lambder = initLambder().create({
    apiPath: "/api",
    guards: {
        // The lambda:InvokeFunction grant on the caller's role is the real
        // authorization; this asserts the request arrived as an invoke, so a
        // misrouted event fails loudly instead of running.
        invokeOnly: lambderGuard({
            handler: async (ctx) => {
                if (ctx.header("x-lambder-invoke") !== "1") refuse("Not an invoke.");
            },
        }),
    },
...
runtimes (`nodejs18.x` and later) already provide the AWS SDK v3, so a function
...
| `ip` | The per-call `clientIp`, forwarded as `X-Forwarded-For`; empty when the call did not supply one |
...
| `client` | none | A ready `LambdaClient`, e.g. one shared with the rest of the app |
| `clientConfig` | `{}` | Otherwise the client is built from this on the first call (region, credentials, `maxAttempts`) |
...
Retries stay with the SDK (`clientConfig.maxAttempts`), which retries throttles
and service errors, answered before the callee ever runs, and never retries a
function that failed. A caller `timeoutMs` only bounds the wait: a synchronous
invoke cannot be cancelled, so a callee that keeps working after the caller
gave up still finishes its work.
...
| `clientIp` | Forwarded as `X-Forwarded-For`, so `ctx.ip` on the callee is the end user's address |
...
failure to `null` so a UI keeps rendering, and the difference is deliberate: a
...
A success outcome carries `payload`, the parsed envelope as `response`, and
`logList`. A failure carries `reason`, always an `error` (the
`LambderInvokeError` `api()` would have thrown) and `logList`, plus whatever
the failure knew: `status`, `errorMessage`, `retryAfterSeconds`, `zodError`,
`response` (the parsed envelope, including on a 5xx when the callee sent one),
`crash`, `functionError` and `bytes`.
...
| `reason` | Meaning |
| --- | --- |
| `network` | The invoke itself failed: the SDK rejected, or an external signal aborted it |
| `timeout` | `timeoutMs` elapsed and the invoke was aborted |
...
| `errorMessage` | A structured refusal (`refuse()`, `LambderApiError`); `errorMessage` carries it |
...
generic message to everyone else. `LambderCaller` ignores the field, so adding
it breaks nothing for the browser.
...
- `isLambderInvokeError(err)` detects it through a brand, so it survives two
  copies of the package, the way `isLambderApiError` does.
...
(unguessable, 16 to 200 characters, one per logical operation). It matters more
here than it looks: the SDK's own retries happen before the callee runs, so
they do not double-execute anything, but an application-level retry after a
timeout may well reach a callee that is still working.
...
like an API call's, named `GET /health`.
...
(`getRemainingTimeInMillis`, `awsRequestId` and the rest are filled in).
