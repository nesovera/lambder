...
describe('HTTP API v2 events', () => {
    ...
    it('applies ETag + If-None-Match 304 on v2 GETs, carrying the call\'s cookies and headers', async () => {
        ...
        expect((second.headers as Record<string, string>).ETag).toBe(etag);
        // v2 carries Set-Cookie in its own array, and a revalidation is still
        // the call that slid the session cookie.
        expect(second.cookies?.[0]).toContain('LMDRSESSIONTKID=slid');
        expect((second.headers as Record<string, string>)['X-Request-Id']).toBe('abc');
        // The body headers go, because a 304 carries no body.
        expect((second.headers as Record<string, string>)['Content-Type']).toBeUndefined();
    });
    ...
    it('emits the v2 shape from the global error handler and the last-resort 500', async () => {
        ...
        const bare = new Lambder({ files: testPublicFiles() })
            .addRoute('/boom', () => { throw new Error('boom'); });
